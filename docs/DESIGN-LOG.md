...
- **Cross-repo artifact (flagged).** The GLB now exists in two places: the CAD
  repo (`docs/models/`, generated by `build.py`) and the manual repo
  (`public/models/`, the served copy). To refresh after a geometry change,
  re-copy the built GLB into the manual's `public/models/`. Chose the on-site
  copy over loading the GitHub Pages URL cross-origin so the manual doesn't
  depend on the CAD Pages site staying up.
